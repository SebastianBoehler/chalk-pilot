...
## Task 1: Make camera setup work equally well at home and in a lecture hall
...
- [ ] Add a failing component test proving the generic heading and
  board-focused home guidance are visible.
- [ ] Rename the heading to “Connect a camera”.
- [ ] When `board-focused` is selected, explain that a nearby webcam or iPhone
  Continuity Camera can point at a whiteboard or flip chart and that presenter
  tracking is skipped.
- [ ] When `room-wide` is selected, retain the auditorium manual-wide-view
  guidance and explain that presenter tracking is enabled.
...
- [ ] Document the supported artifact catalog, the explicit HTML/JavaScript
  exclusion, and the two camera setups:
  lecture-hall room-wide camera and home board-focused/Continuity Camera.
...
- Home iPhone/flip-chart setup requires no speaker tracking; lecture-hall setup
  retains confirmed presenter tracking.

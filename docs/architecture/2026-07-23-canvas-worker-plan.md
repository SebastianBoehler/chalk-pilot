...
- [ ] Write a failing test that creates a temporary workspace, upserts a new
  section, upserts the same ID again, focuses it, and asserts one updated
  section remains.
- [ ] Run `npm test -- src/features/canvas-worker/actions.test.ts` and confirm
  it fails because the module does not exist.
- [ ] Implement the Zod job schemas and minimal repository-backed actions.
...
- [ ] Write failing provider tests for the OpenAI default, explicit OpenRouter,
  missing credentials, and unsupported providers.
- [ ] Run the focused provider test and confirm the missing module failure.
- [ ] Install `ai@^7`, `@ai-sdk/openai@^4`, and
  `@openrouter/ai-sdk-provider@^3`.
- [ ] Implement provider selection with explicit validation. Configure
  OpenRouter with `sort: "throughput"`, `require_parameters: true`,
  `data_collection: "deny"`, and `zdr: true`.
- [ ] Implement a maximum six-step `ToolLoopAgent` with only read, upsert, and
  focus tools and a concise structured completion summary.
- [ ] Run the provider tests and `npm run typecheck`.
...
- [ ] Write a failing queue test proving two jobs for one session finish in
  order while jobs for different sessions may overlap.
- [ ] Implement the smallest keyed promise queue and make the test pass.
- [ ] Write failing API tests for a successful injected worker, invalid input,
  missing session, and provider failure.
- [ ] Implement the service, error mapping, and Node.js route.
- [ ] Run the focused queue and API tests.
...
- [ ] Write failing tool tests proving delegation returns a job ID and direct
  append/update tools are no longer exposed to the voice agent.
- [ ] Write a failing session test proving the tool returns before the worker
  request completes, then applies the returned canvas and adds a private
  completion item without creating a spoken response.
- [ ] Implement background job dispatch, completion context, and independent
  error reporting.
- [ ] Update the tutor instructions to delegate durable artifacts.
- [ ] Run the realtime tool and session tests.
...
- [ ] Write a failing component test that renders the `building` state and
  expects `Canvas worker` plus `Building visual context…`.
- [ ] Thread worker state from the session controller into the workspace and
  make the test pass.
- [ ] Document provider configuration, asynchronous behavior, and the updated
  privacy boundary.
- [ ] Run `npm test`.
...
- [ ] Run `npm run format`.
- [ ] Run `npm run check` under Node 24.
- [ ] Run `npm run test:e2e`.
- [ ] Start the worktree app on a non-conflicting port and verify one mocked
  canvas-worker job through the browser flow.
- [ ] Review the diff for generated files, credentials, and files over 300
